...
- [x] 后端新增 editLogs.revert 接口（接收 logId，将字段值还原为 old_value，并写入新的历史记录）
- [x] 撤销操作本身也写入历史（字段：new_value→old_value，操作人：当前用户，备注：撤销）
- [x] 前端编辑历史页面每条记录右侧添加“撤销”按钮，带确认弹窗
- [x] 撤销成功后刷新历史列表
- [x] 防止重复撤销（已被撤销的记录禁用撤销按钮）
- [x] Vitest 测试覆盖撤销逻辑（32 项测试全部通过）
